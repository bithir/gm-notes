...
class GMNote extends FormApplication {
	...
	static showGMNoteWindow() {
		...
		if (gmNoteObject?.constructor.name === 'JournalEntryPage') {
			...
			page = gmNoteObject.pages.get(
				gmNoteObject._sheet.pagesInView[0]?.dataset?.pageId
			);
		}
	...
	async getData() {
		...
		data.journalNotes = await foundry.applications.ux.TextEditor.enrichHTML(
			this.object.getFlag('gm-notes', 'notes'),
			{ async: true }
		);
	...
	getCurrentPage() {
		...
		// Find current page
		let pageIdentifier = this.object.sheet.pagesInView[0]?.dataset?.pageId;
	...
	activateListeners(html) {
		...
		elem
			.closest('.window-app')
			.querySelector('header > .window-title').textContent = game.i18n.format(
			'GMNote.title',
			{
				document:
					this.object.name ?? `${this.object.documentName}[${this.object.id}]`,
			}
		);
	}
	...
	static async _addContentToJournal(app, html, data) {
		...
		const enrichedContent = await TextEditor.enrichHTML(gmNotes, {
			async: true,
		});
	...
	static _updateHeaderButton(app, [elem], options) {
		...
		setTimeout(() => {
			...
			gmNotesButton.innerHTML = `<i class="fas ${
				notes ? 'fa-clipboard-check' : 'fas fa-clipboard'
			}"></i> ${
